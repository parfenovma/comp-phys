...
Вычисление сеточной функции $p(l)$ по найденным амплитудам гармоник проводится по формуле:
...
### Графики результатов
Вычисления производились на сетке с шагом $h/10$. Видно, что интерполированная кривая проходит через исходные узлы дискретизации и совпадает с исходной аналитической функцией, что доказывает теорему Котельникова (рис. 4.a, 4.b).
...
Видно, что для каждой из точек ошибка не превышает машинную точность (порядка $10^{-16}$).

![Оценка ошибки](pic/fig_4_error.png)
...
plot(t_l, p_l, 'Исходный дискретный p(l)');
hold on;
plot(t_l, p_restored_real, 'Восстановленный ОДПФ');
hold off;
...
plot(t_l, p_l, 'ko', 'Узлы дискретизации p(l)');
hold on;
plot(t_fine, p_interpolated, 'Интерполяция (sinc)');
plot(t_fine, p_analytical_fine, 'Аналитический сигнал');
hold off;
